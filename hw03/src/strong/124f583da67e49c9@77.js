...
Inputs.color({ label: "stroke color", value: "#c196e4" })
)}
...
(width, height, offsetX, offsetY, scale) => {
  ...
  // 設定顏色
  const startColor = d3.rgb(253, 203, 241);     // 起始顏色（RGB 格式）
  const endColor = d3.rgb(230, 222, 233);       // 結束顏色（RGB 格式）
  const colorRange = thresholds.map(value => d3.interpolateRgb(startColor, endColor)(value / maxValue));
  
  // 手動指定顏色陣列
  const customColorRange = ["#fdebf1", "#fddaf1", "#fdcbf1", "#fdbbf1", "#fdaaf1", "#fd94f1"];
  
  const thresholdScale = d3.scaleThreshold().domain(thresholds).range(customColorRange);
